...
export default function Options() {
  ...
  const fetchOptions = async () => {
    setLoading(true);
    try {
      const getOptions = await getAllOptions();
  ...
  return (
    <Box>
      <Typography variant="h5" gutterBottom>Options List</Typography>
        ...
        <Paper sx={{ width: '100%', overflow: 'auto' }}>
          <TableContainer>
            <Table stickyHeader>
              ...
              <TableBody>
                {options.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, idx) => (
                  <TableRow
                    hover
                    key={idx}
